...
/**
 * Helper to format lint messages consistently.
 */
function msg(tag: string, issue: string): string {
  ...
  const parser = new Parser({
    ...
    onopentag(name, attrs) {
      ...
      if (['input','select','textarea'].includes(tag)) {
        ...
        if (!aria || !aria.trim()) {
          if (!id) results.push({ ...pos, message: msg('<form control>', 'missing "id" or "aria-label"') });
          else if (!labels.has(id)) results.push({ ...pos, message: msg('<form control>', `input with id=\"${id}\" missing <label for=\"${id}\">`) });
        }
      }
      // only one h1
      if (tag === 'h1') {
        h1Count++;
        if (h1Count > 1) results.push({ ...pos, message: msg('<h1>', 'only one <h1> allowed per document') });
      }
      // heading order
      if (/^h[1-6]$/.test(tag)) {
        const lvl = parseInt(tag.charAt(1), 10);
        if (lastHeadingLevel && lvl > lastHeadingLevel + 1) {
          results.push({ ...pos, message: msg('<heading>', `<${tag}> used after <h${lastHeadingLevel}> without <h${lastHeadingLevel + 1}>`) });
        }
      ...
      if (tag === 'img') {
        const alt = attrs.alt;
        if (!alt || !alt.trim()) results.push({ ...pos, message: msg('<img>', 'missing non-empty "alt" attribute') });
      }
      // li nesting
      if (tag === 'li') {
        const parent = tagStack[tagStack.length - 2];
        if (!parent || !['ul','ol'].includes(parent.tag)) {
          results.push({ ...pos, message: msg('<li>', 'must be a direct child of <ul> or <ol>') });
        }
      }
      // anchor
      if (tag === 'a') {
        ...
        if (!href || !href.trim()) results.push({ ...pos, message: msg('<a>', 'missing non-empty "href" attribute') });
      }
      // table caption
    ...
    onclosetag(name) {
      ...
      if (inlineTags.has(tag)) {
        ...
        if (e && !e.foundText) results.push({ line: e.line, column: e.column, message: msg(`<${tag}>`, 'should not be empty') });
      }
      if (tag === 'a') {
        const a = anchorStack.pop();
        if (a && !a.foundText) results.push({ line: a.line, column: a.column, message: msg('<a>', 'missing link text') });
      }
      if (tag === 'section') {
        const s = sectionStack.pop();
        if (s && !s.foundHeading) results.push({ line: s.line, column: s.column, message: msg('<section>', 'must contain a heading (<h1>-<h6>)') });
      }
      if (tag === 'table') {
        const t = tableStack.pop();
        if (t && !t.foundCaption) results.push({ line: t.line, column: t.column, message: msg('<table>', 'missing <caption>') });
      }
    }
    ...
    JSXElement: {
      enter(path: NodePath<t.JSXElement>) {
        ...
        if (['input','select','textarea'].includes(tag)) {
          ...
          if (!ariaVal || !ariaVal.trim()) {
            results.push({ ...pos, message: msg(`<${tag}>`, 'missing "id" or "aria-label"') });
          }
        }
        // only one h1
        if (tag === 'h1') {
        ...
        if (tag === 'img') {
          ...
          if (!hasAlt) results.push({ ...pos, message: msg('<img>', 'missing non-empty "alt" attribute') });
        }
        // li nesting
        if (tag === 'li') {
          ...
          if (t.isJSXElement(parent)) {
            ...
            if (t.isJSXIdentifier(p)) {
              ...
              if (!['ul','ol'].includes(pTag)) results.push({ ...pos, message: msg('<li>', 'must be a direct child of <ul> or <ol>') });
            }
          }
        }
        // anchor href
        if (tag === 'a') {
          results.push({ ...pos, message: msg('<a>', 'missing non-empty "href" attribute') });
        }
      },
      exit(path: NodePath<t.JSXElement>) {
        ...
        if (['strong','em','b','i','u','small','mark','del','ins'].includes(tag)) {
          const e = emptyStack.pop();
          if (e && !e.foundText) results.push({ ...e, message: msg(`<${tag}>`, 'should not be empty') });
        }
      }
    }
  });
